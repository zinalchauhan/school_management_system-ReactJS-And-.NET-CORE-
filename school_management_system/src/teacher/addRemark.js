...
export class teacherAddRemark extends Component {
    ...
    fetch(Variables.TECH_API_URL + "checkClassTeacher/" + tech)
      ...
      .then((res) => {
        if (res.result === "success") {
          ...
          this.getStudent(res.data.classIdFk);
        }
      });
  }
  ...
  getStudent(id) {
    fetch(Variables.TECH_API_URL + "classStudList/" + id , {
      method: "GET",
      headers: {
...
insert(sid) {
  ...
  fetch(Variables.TECH_API_URL + "insertRemarkList", {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
  ...
  render() {
    const { classes, classIdFk, studs , classname} = this.state;
    return (
      <div>
          <Header></Header>
